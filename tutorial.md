...
Die folgenden Schritte sind notwendig, um ein Projekt aufzusetzen, das eine _Single Page Application_ (SPA) in Form einer HTML-Seite erzeugt, das die eingegebenen Zeichen (ohne Leerzeichen) in einem Textfeld zählen kann. Ich nenne es `charcount`. Ich werde es mit TypeScript implementieren.
...
Ich benötige eine `.gitignore`-Datei, die sagt, welche Dateien ich nicht mit Git verwalten möchte, weil sie im Laufe des Projekts erzeugt werden. Aktuell sind es ein Verzeichnis und zwei Dateien. Diese Datei muss jeweils angepasst werden, wenn sie das ändert (das `^D` steht für Ctrl+D, was eingegeben werden muss, um `cat` zu beenden):
